...
## 📂 Structure du projet (Mise à jour avant CRUD Commentaire)
...
├── config/
│   └── packages/
│       ├── security.yaml       # MODIFIÉ (règle access_control pour /admin)
│       └── twig.yaml           # (Configuration form_themes si pertinent)
├── public/
│   # ... (contenu existant) ...
├── src/
│   ├── Controller/
...
│   │   └── PostController.php      # MODIFIÉ (logique modération: isApproved=false, filtre affichage)
│   ├── Entity/
│   │   ├── Article.php           # (Modifié précédemment)
│   │   ├── Comment.php           # (Modifié précédemment)
│   │   ├── Post.php              # (Modifié précédemment)
│   │   ├── User.php              # (Rôle admin via DB pour tests)
│   │   # ... (autres entités) ...
│   ├── Form/
│   │   ├── CommentFormType.php   # (Créé précédemment, bouton submit ajouté)
│   │   └── ContactFormType.php
│   ├── Repository/
│   │   ├── CommentRepository.php # (Créé précédemment, sera utilisé pour requêtes affichage)
│   │   # ... (autres repositories) ...
│   └── Security/
│       └── CustomAuthenticator.php # (Modifié précédemment)
├── templates/
│   ├── post/
│   │   ├── detail.html.twig      # MODIFIÉ (affichage filtré des commentaires)
│   │   └── postindex.html.twig
│   ├── base.html.twig
│   # ... (autres templates) ...
# ... (autres fichiers) ...
