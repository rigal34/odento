    ...
    public function detail(Post $post, Request $request, EntityManagerInterface $entityManager): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            if ($user) { // Vérifie si un utilisateur est bien connecté
                ...
                // Redirection vers la même page pour voir le commentaire et éviter la re-soumission
                ...
                // On redirige vers la même page pour afficher le message flash et "nettoyer" la requête POST
                return $this->redirectToRoute('app_actu_detail', ['id' => $post->getId()]);
                // Alternativement, on pourrait rediriger vers la page de connexion:
                // return $this->redirectToRoute('app_login');
            }
        }

        // Cette instruction render est pour :
        // 1. L'affichage initial de la page (requête GET, le formulaire n'est pas soumis)
        // 2. Si le formulaire est soumis mais n'est PAS valide (Symfony affichera les erreurs)
        // 3. Si le formulaire est soumis, l'utilisateur n'est pas connecté ET on n'a pas redirigé plus haut
        return $this->render('post/detail.html.twig', [
            'post' => $post,
            'comment_form' => $form->createView(), // Utilisation de 'comment_form' comme convenu
        ]);
    }
}
