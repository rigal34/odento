...
final class ContactController extends AbstractController
{
    ...
    // --- AJOUTER Request et MailerInterface EN ARGUMENTS ---
    public function contact(Request $request, MailerInterface $mailer): Response
    {
        // 1. Créer une instance de notre formulaire
        $form = $this->createForm(ContactFormType::class);

        // 2. Permettre au formulaire de traiter les données envoyées dans la requête
        // S'il y a des données POST, le formulaire va les lire et les valider.
        $form->handleRequest($request);

        // 3. Vérifier si le formulaire a été soumis ET si les données sont valides
        // (conformes aux contraintes définies dans ContactFormType)
        if ($form->isSubmitted() && $form->isValid()) {
            
            // --- Formulaire soumis et valide ---

            // 4. Récupérer les données nettoyées du formulaire sous forme de tableau
            $data = $form->getData();
