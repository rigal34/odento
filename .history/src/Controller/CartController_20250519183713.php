   ...
   public function cartIcon(Request $request): Response
   {
    ...
    if (!empty($cart)) {
        foreach ($cart as $quantity) {
            $totalItemCount += $quantity; // Compter le nombre total d'articles dans le panier
        }
    }

    return $this->render('cart/_cart_icon.html.twig', [
...
//CREATION de mon bouton mise à jour du panier en version formulaire
   #[Route('/panier/mettre-a-jour', name: 'app_cart_update', methods: ['POST'])]
    ...
    public function updateQuantities(Request $request): RedirectResponse
   {
    ...
    $submittedQuantities = $postData['quantities'] ?? []; // Récupérer toutes les données soumises du formulaire

    if (is_array($submittedQuantities)) {
        foreach ($submittedQuantities as $id => $newQuantity) {
            $id = (int)$id; // Convertir l'ID en entier
            $newQuantity = (int)$newQuantity; // Convertir la quantité en entier
        
            if ($newQuantity > 0) {
                $cart[$id] = $newQuantity; // Mettre à jour la quantité dans le panier
            } else {
                unset($cart[$id]); // Supprimer l'article du panier si la quantité est 0 ou négative
            }
    }
//   dump($submittedQuantities);

//   dd('Le formulaire de mise à jour a été soumis avec succès !');
